...
at `/snap/core20/current/usr/share/doc/ChangeLog`.

You can list all installed revisions with `snap list --all core20`. To inspect
a specific revision that is not the active one, replace `current` with the
revision number (e.g. `/snap/core20/1234/usr/share/doc/ChangeLog`).
...
unsquashfs -d core20-unpacked core20_*.snap usr/share/doc/ChangeLog
```

The ChangeLog is then available at `core20-unpacked/usr/share/doc/ChangeLog`.
